...
SESSION_DIR = None
OUTPUT_DIR  = None
#SESSION_DIR = r"C:\Users\Magdalena\OneDrive\Escritorio\Trials_Ruth"
...
def extract_valgus_series(trc_path, side, phase_window=(50, 60)):
    try:
        df_trc = load_trc_opensim(trc_path)
        result = pdkv_by_stride_from_trc(df_trc, side, phase_window=phase_window)
        vals = np.asarray(result["peaks_deg"], dtype=float).ravel()
...
def plot_kinematic_variability_per_trial(data_dict, trial_names,
    ...
    # Guardar en PNG y PDF (usa la misma base de nombre, sin extensión)
...
def _save(fig, output_dir, name, dpi=300, subfolder=None):
...
def plot_scalar_per_trial_series(per_trial_R, per_trial_L, trial_names,
    ...
    TRIAL_COLORS = [
        "#14991b", "#ff0066", "#2ecc71", "#9b59b6",
        "#e74c3c", "#1abc9c", "#f39c12", "#2100fb",
    ]
...
def plot_poincare(series_R, series_L, output_dir):
    ...
    for ax, series, side in zip(axes, [series_L, series_R], ["L", "R"]):
        if series.size < 4:
            ax.set_visible(False)
            continue
...
def main():
    ...
    # ── 2. Series por ensayo (ciclo a ciclo) ─────────────────
