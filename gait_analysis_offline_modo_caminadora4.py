...
from Gráficos_adicionales_modo_caminadora import generate_additional_report
...
try:
    from fall_risk_entropy_modo_caminadora4 import (
...
try:
    from gait_analysissinopensim12 import gait_analysis as _gait_analysis_impl
except (ImportError, AttributeError):
    try:
        from gait_analysissinopensim5 import gait_analysis as _gait_analysis_impl
...
def pdkv_from_markers(df: pd.DataFrame, hip, knee, ankle, toe, phase=(50, 60)):
...
def build_ga_robust(session_dir, trial_name, leg, filter_frequency, gait_style,treadmill_speed_override=None):
    # Primero intentar con todos los ciclos y distintos trimmings
    trim_options = [
        (0.05, 0.05),
        (0.10, 0.10),
        (0.15, 0.10),
    ...
    ERROR_KEYWORDS = [
        "ordering of gait events is not correct",
        "not enough gait cycles",
        "no good steps",
    ]

    def is_segmentation_error(e):
        msg = str(e).lower()
        return any(kw.lower() in msg for kw in ERROR_KEYWORDS)

    last_err = None
    for ts, te in trim_options:
        for n_cycles in [-1, 4, 3, 2]:
            try:
                ga = gait_analysis(
                    ...
                    trimming_start=ts, trimming_end=te,
                    treadmill_speed_override=treadmill_speed_override
                )
                #print(f"[DEBUG buildga] treadmill_speed_override={treadmill_speed_override}, ga.treadmillSpeed={ga.treadmillSpeed:.4f}")
                return ga, ts, te
            except Exception as e:
                last_err = e
                msg = str(e).lower()
                # Reintentar si es error de segmentación o de ciclos no encontrados
                if ("ordering of gait events" not in msg and 
                    "only -1 were found" not in msg and
                    "negative dimensions" not in msg):
                    raise

    raise RuntimeError(
        f"No se pudo segmentar automáticamente la marcha para la pierna {leg}.\n"
        f"El ensayo puede tener muy pocos pasos o marcadores de mala calidad.\n"
...
# Pipeline principal
# =========================================================
def run_full_analysis_offline_colors(trc_path, mot_path, edad, sexo, altura_m, treadmill_speed=0.0):
    ...
    try:
        _ts = float(treadmill_speed)
    except (TypeError, ValueError):
        _ts = 0.0

    ts_override = _ts if _ts > 0 else None

    ga_r = gait_analysis(
        session_dir, trial_name, leg='r',
        lowpass_cutoff_frequency_for_coordinate_values=filter_frequency,
        n_gait_cycles=-1,
        gait_style='treadmill',
        trimming_start=trim_start,
        trimming_end=trim_end,
        treadmill_speed_override=ts_override
    )

    ga_l = gait_analysis(
        session_dir, trial_name, leg='l',
        lowpass_cutoff_frequency_for_coordinate_values=filter_frequency,
        n_gait_cycles=-1,
        gait_style='treadmill',
        trimming_start=trim_start,
    ...
    def arr_mean(scal_dict, name):
        v = scal_dict[name]['value']
        a = np.asarray(v, dtype=float).ravel()
        a = a[np.isfinite(a)]
        return float(np.mean(a)) if a.size > 0 else np.nan
    
    def arr_max(scal_dict, name):
        v = scal_dict[name]["value"]
        a = np.asarray(v, dtype=float).ravel()
        a = a[np.isfinite(a)]
        return float(np.max(a)) if a.size > 0 else np.nan

    scal_r_all = ga_r.compute_scalars(scalar_names, return_all=True)
    scal_l_all = ga_l.compute_scalars(scalar_names, return_all=True)

    #print("[DEBUG stride R all]", np.asarray(scal_r_all["stride_length"]["value"]).ravel())
    #print("[DEBUG stride L all]", np.asarray(scal_l_all["stride_length"]["value"]).ravel())
    #print("[DEBUG stride R mean]", arr_mean(scal_r_all, "stride_length"))
    #print("[DEBUG stride L mean]", arr_mean(scal_l_all, "stride_length"))
    ...
    stepw_r_1 = arr_mean(scal_r_all, "step_width")
    step_length_symmetry_r = arr_first(scal_r_all, "step_length_symmetry")
    step_length_symmetry_r_mean = arr_mean(scal_r_all, "step_length_symmetry")
    double_support_time_r = arr_first(scal_r_all, "double_support_time")
    double_support_time_r_mean = arr_mean(scal_r_all, "double_support_time")
    ...
    metrics = {
        ...
        "Doble apoyo [%]":                  double_support_time_r,
        "Simetría longitud paso [%]":            step_length_symmetry_r,
    }
    ...
    for i, (metric, val) in enumerate(metrics.items()):
        if "Ancho" in metric:
            ...
            try:
                color_bar_speed(axs[i], val, mean, sd, metric, vmax=4.0)
            except TypeError:
                val_plot = min(float(val), 3.9)
                color_bar_speed(axs[i], val_plot, mean, sd, metric)
                if val_plot < val:
                    axs[i].text(0.98, 0.12, f"{val:.2f} m/s",
        ...
        elif "Doble" in metric:
            color_bar_double_support(axs[i], val, metric)
        elif "Simetría" in metric:
            color_bar_symmetry(axs[i], val, metric)
...
def build_bullets(patient):
    ...
    return [
        ...
        ("Doble apoyo", "Corresponde al tiempo en que ambos pies están en contacto con el suelo durante la marcha. Un valor menor al 35% del ciclo se considera adecuado."),
        ("Simetría de la longitud del paso", "Corresponde a la relación entre la longitud del paso derecho e izquierdo. Un valor entre 90 y 110 se considera adecuado."),
    ]
...
def export_pdf_report(save_path, logo_path, patient, trial_name, fig_results, metrics,
    ...
    with PdfPages(save_path) as pdf, tempfile.TemporaryDirectory() as tmpdir:
        ...
        patient_info = [f"Paciente: {patient.get('nombre','')}", f"Edad: {patient.get('edad','')} años",
                        f"Altura: {patient.get('altura','')} m", "Modo: Caminadora"]
        ...
        if fall_risk_result:
            ...
            _entropy_note = _fr.get("_entropy_note", "")
            _n_trials = fall_risk_result.get("n_trials", 1)

            _sb = _fr.get("score_base")
            _sf = _fr.get("score_final")
            _sse = _fr.get("score_sampen")
            _sauc = _fr.get("score_auc_mse")
            _s_entropy_total = _fr.get("score_entropy_total")
            entropy_level = _fr.get("_entropy_level")
            _fh = _fr.get("fall_history_applied", False)
            _contributing = _fr.get("contributing_factors", [])

            st_se  = _ent.get("stride_time", {}).get("sampen")
            st_auc = _ent.get("stride_time", {}).get("mse", {}).get("auc_mse")
            st_n   = _ent.get("stride_time", {}).get("n")

            stv_se = _ent.get("stride_time_variability", {}).get("sampen")
            stv_n  = _ent.get("stride_time_variability", {}).get("n")

            ta_auc = _ent.get("trunk_ap_acceleration", {}).get("mse", {}).get("auc_mse")
            ta_n   = _ent.get("trunk_ap_acceleration", {}).get("n")

            _s_combined = None
            if _sb is not None and _s_entropy_total is not None:
                try:
                    _s_combined = 0.60 * float(_sb) + 0.40 * float(_s_entropy_total)
                except (TypeError, ValueError):
                    _s_combined = None
            ...
            _ent_level, _ent_color = _ent_risk_level(_s_entropy_total)
            ...
            _hist_tag = f" (+15 pts historial -> {_fe2(_sf)}/100)" if _fh else ""
            _score_clinico_txt = f"{_fe2(_sb)}/100{_hist_tag}" if _sb is not None else "N/D"
            _score_sampen_txt = f"{_fe2(_sse)}/50" if _sse is not None else "N/D"
            _score_auc_txt = f"{_fe2(_sauc)}/50" if _sauc is not None else "N/D"
            _score_ent_txt = f"{_fe2(_s_entropy_total)}/100" if _s_entropy_total is not None else "N/D"
            _score_comb_txt = f"{_s_combined:.1f}/100" if _s_combined is not None else "N/D"
            ...
            _banner.text(0.055, 0.48, f"Riesgo {_ent_level}", color=_ent_color,
                         fontname="Cambria", fontsize=12, fontweight="bold", va="center", ha="left")
            _banner.text(0.3, 0.48, f"Score Entropía: {_score_ent_txt}", color="#555555",
                         fontname="Cambria", fontsize=9.5, va="center", ha="left")
            ...
            _stats_rows = [
                ...
                ("double_support_mean_pct", None, "double_support_cv_pct", "Doble apoyo", "%"),
                ("step_length_mean_m", None, None, "Longitud de paso media", "m"),
                (None, None, "stride_time_cv_pct", "CV Tiempo de zancada", "%"),
            ]
            ...
            if _contributing:
                ...
                _p2_map = [
                    (
                        ...
                        "· Variabilidad del tiempo de zancada aumentada: es la variable con mayor peso en el score de "
                        "caminadora por su fuerte asociación con riesgo de caída. Refleja pérdida de regulación fina del "
                        "ritmo locomotor sobre la cinta."
                    ),
                    (
                        ("longitud de paso",),
                        "· Longitud de paso reducida: una longitud de paso baja respecto a la talla del paciente sugiere "
                        "una estrategia de marcha más cautelosa y menor capacidad de propulsión en caminadora."
                    ),
                    (
                        ("cadencia",),
                        "· Alteración en cadencia: cadencias fuera del rango habitual (95–120 pasos/min) se asocian a "
                        "inestabilidad y menor capacidad de sostener un ritmo locomotor estable durante la marcha en cinta."
                    ),
                    (
                        ("doble apoyo",),
                        "· Aumento del tiempo de doble apoyo: variable secundaria con fuerte respaldo en literatura "
                        "prospectiva. Un mayor doble apoyo refleja una estrategia compensatoria de seguridad y menor "
                        "confianza para transferir carga entre miembros."
                    ),
                ]
            ...
            if _combined_exp:
                ...
                _parts = []
                _p2_map = [
                    (("cv stride time", "variabilidad"),
                     "- CV tiempo de zancada elevado: predictor más robusto de caidas (RR 1.007/unidad, Hausdorff 2001)."),
                    (("velocidad", "speed"),
                     "- Velocidad de marcha reducida (<1.12 m/s): mayor mortalidad y riesgo de caída (Studenski 2011)."),
                    (("stride length", "longitud"),
                     "- Longitud de zancada reducida: OR significativo en modelos multivariantes (Oberg 1993)."),
                    (("cadencia", "cadence"),
                     "- Alteracion en cadencia (CV >8%): inestabilidad y mayor riesgo prospectivo (Kressig 2004)."),
                    (("cv stride length",),
                     "- CV largo de zancada elevado (>6%): perdida de consistencia espacial (Hausdorff 2001)."),
                ]
                if _contributing:
                    for _fi in _contributing:
                        _fi_low = _fi.lower()
                ...
                if _ent_st:
                    ...
                    if _se_v is not None:
                        if _se_v < 0.8:
                            _parts.append(f"- SampEn stride time ({_se_v:.3f}) <0.8: rigidez locomotora (Stergiou 2011).")
                        elif _se_v > 2.8:
                            _parts.append(f"- SampEn stride time ({_se_v:.3f}) >2.8: perdida de control del patron (Stergiou 2011).")
                        else:
                            _parts.append(f"- SampEn stride time ({_se_v:.3f}) en rango normal (0.8-2.8): complejidad preservada.")
                    if _mse_v is not None:
                        if _mse_v < 0.71 or _mse_v > 1.40:
                            _parts.append(f"- AUC-MSE ({_mse_v:.3f}) fuera de rango (0.71-1.40): perdida de complejidad multiescala (Lipsitz 2004).")
                        else:
                            _parts.append(f"- AUC-MSE ({_mse_v:.3f}) en rango normal (0.71-1.40): complejidad multiescala preservada.")
                _parts.append("Score acumulado basado en: Hausdorff 2001, Studenski 2011, Kressig 2004, "
                               "Tinetti 1988, Stergiou 2011, Costa 2002, Lipsitz 2004.")
                _combined_text = "\n".join(_parts)
...
def main_gui():
    ...
    ttk.Label(frm, text="Género:", background="white").grid(row=3, column=0, sticky='e', padx=6, pady=4)
    ttk.Combobox(frm, textvariable=vars['sexo'], values=["Hombre","Mujer"], state="readonly", width=12).grid(row=3, column=1, sticky='w', padx=6, pady=4)

    ttk.Label(frm, text="Modo:", background="white").grid(row=4, column=0, sticky='e', padx=6, pady=4)
    ttk.Label(frm, text="Caminadora", background="white").grid(row=4, column=1, sticky="w", padx=6, pady=4)

    ttk.Label(frm, text="Velocidad caminadora (m/s):", background="white").grid(row=5, column=0, sticky='e', padx=6, pady=4)
    vars['treadmill_speed'] = tk.StringVar(value="")
    ttk.Entry(frm, textvariable=vars['treadmill_speed'], width=10).grid(row=5, column=1, sticky='w', padx=6, pady=4)

    # Historial de caída 12 meses
    ttk.Label(frm, text="Caída últimos 12 meses:", background="white").grid(row=6, column=0, sticky="e", padx=6, pady=4)
    ttk.Combobox(frm, textvariable=vars["fall_history_12m"],
                values=["Sí","No"], state="readonly", width=5).grid(row=6, column=1, sticky="w", padx=6, pady=4)

    for i, ext in enumerate(["trc","mot"]):
        ttk.Label(frm, text=f"Archivo .{ext.upper()}:", background="white").grid(
            row=7+i, column=0, sticky="e", padx=6, pady=4)
        ttk.Entry(frm, textvariable=vars[ext], width=60).grid(
            row=7+i, column=1, sticky="we", padx=6, pady=4)
        ...
        ttk.Button(frm, text="Examinar…", command=cmd).grid(row=7+i, column=2, padx=6, pady=4)

    # ── Carpeta de sesión (opcional, para entropía) ──
    ttk.Label(frm, text="Carpeta sesión\n(entropía):", background="white",
              justify="right").grid(row=9, column=0, sticky="e", padx=6, pady=4)
    ...
    ttk.Button(frm, text="Examinar…", command=_browse_session).grid(row=9, column=2, padx=6, pady=4)
    ...
    ttk.Label(frm, text="*Al cargar la carpeta también puedes generar gráficos adicionales. ",
              background="white", foreground="#666", font=("TkDefaultFont",8)).grid(
        row=13, column=1, columnspan=3, sticky="w", padx=6)

    btn = ttk.Button(frm, text="Ejecutar análisis"); 
    btn.grid(row=15, column=0, columnspan=3, pady=10)
    ...
    export_png_btn = ttk.Button(export_buttons_frame, text="Export PNG", state="disabled")
    ...
    # ── Panel de riesgo de caída (en la pestaña Resultados) ──
    ...
    def ejecutar():
        ...
        try:
            ...
            try:
                _tspd = float(vars['treadmill_speed'].get().replace(',', '.'))
            except (ValueError, TypeError):
                _tspd = 0.0

            fig_final, info = run_full_analysis_offline_colors(
                trc, mot, age, vars['sexo'].get(), hgt,
                treadmill_speed=_tspd
            )
            ...
            if fall_risk_result:
                ...
                _entropy_note = fr.get("_entropy_note", "")
                ...
                _s_auc= fr_.get("score_auc_mse")
                _fh   = fr_.get("fall_history_applied", False)
                _ntr     = fall_risk_result.get("n_trials", 1)
                ...
                stat_rows = [
                    ...
                    ("stride_time_cv_pct",  "Tiempo de zancada (CV)"),
                    ("step_length_mean_m",  "Longitud de paso (media)"),
                    ("step_length_cv_pct",  "Longitud de paso (CV)"),
                    ("cadence_mean_spm",    "Cadencia (media)"),
                    ("cadence_cv_pct",      "Cadencia (CV)"),
                    ("double_support_mean_pct", "Doble apoyo (media)"),
                    ("double_support_cv_pct", "Doble apoyo (CV)"),
                ]
                ...
                for left_, right_ in pairs:
                    ...
                    _unit_map = {
                        ...
                        "stride_time_cv_pct": " %",
                        "step_length_mean_m": " m",
                        "step_length_cv_pct": " %",
                        "cadence_mean_spm": " pasos/min",
                        "cadence_cv_pct": " %",
                        "double_support_mean_pct": " %",
                        "double_support_cv_pct": " %",
                    }
                ...
                if _lv_es == "ALTO":
                    p1 = (
                        "  El perfil cinemático de este paciente en caminadora presenta múltiples "
                        "indicadores de riesgo elevado de caída. Un score ≥50 puntos, especialmente con "
                        "historial de caídas previas (+15 puntos), refleja una combinación de "
                        "alteraciones en variabilidad del tiempo de zancada, longitud de paso, "
                        "cadencia y/o doble apoyo que la literatura asocia a un riesgo significativamente "
                        "aumentado (Hausdorff 2001, Kressig 2004, Tinetti 1988).\n"
                    )
                elif _lv_es == "MODERADO":
                    p1 = (
                        "  El perfil cinemático en caminadora muestra señales de alerta en algunos "
                        "parámetros de marcha, sin alcanzar el umbral de riesgo alto. Un score en rango "
                        "moderado (25–49 puntos) indica que el paciente presenta alteraciones parciales "
                        "que pueden requerir seguimiento clínico y valoración funcional periódica "
                        "(Lord 2011, Maki 1997).\n"
                    )
                else:
                    p1 = (
                        "  El perfil cinemático en caminadora se encuentra dentro de los rangos "
                        "normativos para la mayoría de los parámetros evaluados. Un score bajo "
                        "(<25 puntos) indica una marcha estable con dinámica locomotora preservada. "
                        "Se recomienda mantener seguimiento preventivo anual (Studenski 2011, Cesari 2005).\n"
                    )
                ...
                if factors_:
                    ...
                    for fi_ in factors_:
                        fi_low = fi_.lower()
                        if "variabilidad del tiempo de zancada" in fi_low:
                            p2_lines.append(
                                ...
                                "regulación del ritmo locomotor durante la marcha en caminadora.\n"
                            )
                        elif "longitud de paso" in fi_low:
                            p2_lines.append(
                                "  · Longitud de paso reducida: una longitud de paso baja respecto a la "
                                "talla del paciente predice mayor riesgo de caída en modelos "
                                "multivariantes (Oberg 1993, Bohannon 1997).\n"
                            )
                        elif "cadencia" in fi_low:
                            p2_lines.append(
                                "  · Alteración en cadencia: cadencias fuera del rango 95–120 pasos/min "
                                "se asocian a inestabilidad y mayor riesgo prospectivo de caídas "
                                "(Kressig 2004, Maki 1997).\n"
                            )
                        elif "doble apoyo" in fi_low:
                            p2_lines.append(
                                "  · Doble apoyo aumentado: un mayor porcentaje de tiempo en doble apoyo "
                                "refleja una estrategia compensatoria de estabilidad y se asocia a mayor "
                                "riesgo de caída en estudios prospectivos (Lord 2011).\n"
                            )
                ...
                if ent_st_:
                    ...
                    p3_parts = []
                    if se_val is not None and not (isinstance(se_val, float) and se_val != se_val):
                        if se_val < 1.00 or se_val > 1.70:
                            p3_parts.append(
                                ...
                                "complejidad locomotora en caminadora (Stergiou 2011, Hausdorff 2001).\n"
                            )
                    ...
                    if mse_val is not None and not (isinstance(mse_val, float) and mse_val != mse_val):
    ...
    dataset_folder = {"path": None}  # mutable para cerrar sobre él
