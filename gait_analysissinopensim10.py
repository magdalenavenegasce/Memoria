"""
    ...
    Unificación de v6 (Sobre suelo) y v7 (Caminadora) en un único archivo.

    Cambios respecto a v6/v7:
      - compute_step_width: usa los markers correctos de v7 (ankle+mankle)
        y restaura la proyección al gait frame de v6 (con bug-fix: ()
        faltaban en self.R_world_to_gait). Funciona correctamente en ambos modos.
      - compute_treadmill_speed: versión v7 (más limpia, maneja ambos modos).
      - segment_walking: versión v7 (deduplicación de TOs, mejora caminadora
        sin afectar sobresuelo).
      - __init__: acepta treadmill_speed_override (v7); eliminada variable
        muerta `n = self.nGaitCycles`.
      - compute_stride_length: eliminados prints de debug de v7.
